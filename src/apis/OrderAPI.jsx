...
export const OrderAPI = {
  /** Get orders from the database that meets the specified query parameters
   */
  getAll: async (conditionObj = false, page = 0, size = 5, search = false, authInfo, cancel = false) => {
    const queryParam = conditionObj ? `&${conditionObj.key}=${conditionObj.value}` : '';
    const searchParam = search ? `&search=${search}` : '';
    const { data: response } = await api.request({
      url: `/get-orders?page=${page}&size=${size}${queryParam}${searchParam}`,
      // headers: {
   ...
   */
  createOrder: async (order, cancel = false) => {
    return await api.request({
      ...
      method: "POST",
      data: order,
      signal: cancel ? cancelApiObject[this.createOrder.name].handleRequestCancellation().signal : undefined,
    });
  },

  /** Update Order in the database
   */
  updateOrder: async (fields, cancel = false) => {
    return await api.request({
      url: `/update-Order`,
      // headers: {
      //   "x-access-token": getLocalStorage("auth").token
